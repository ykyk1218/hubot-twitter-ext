...
class TwitterStreaming extends EventEmitter
 ...
 tweet: (track, callback) ->
   @post @domain, "/1.1/statuses/filter.json?track=#{track}", '', callback
 ...
 post: (domain, path, body, callback) ->
   @request "POST", domain, path, body, callback

 request: (method, domain, path, body, callback) ->
